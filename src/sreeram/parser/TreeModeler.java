...
public class TreeModeler {
	...
	public void changeLeaf(Tree t) {
		if (t.getChildrenAsList().size() != 0) {
			...
			if (t.nodeString().equalsIgnoreCase("sweden") || t.nodeString().equalsIgnoreCase("russian")
					|| t.nodeString().equalsIgnoreCase("canadian")) {
			...
			} else if (t.nodeString().equalsIgnoreCase("strikes") || t.nodeString().equalsIgnoreCase("win")
					|| t.nodeString().equalsIgnoreCase("arrive")) {
	...
	public boolean isSameArray(List<Tree> base, List<Tree> other) {
		if (base.size() != other.size())
			...
			for (int count = 0; count < base.size(); count++) {
				...
				if (!base.get(count).label().value().equalsIgnoreCase(other.get(count).label().value()))
					if (!(base.get(count).label().value().equalsIgnoreCase("NNP") || base.get(count).label().value().equalsIgnoreCase("NNPS"))
							&& (other.get(count).label().value().equalsIgnoreCase("NNP") || other.get(count).label().value().equalsIgnoreCase("NNPS")))
						return false;
	...
	public void getDBMap(Tree parseTree, Tree ruleTree) {
		if (ruleTree.getChildrenAsList().size() != 0 && parseTree.getChildrenAsList().size() != 0) {
			...
			if ((ruleTree.label().value().contains("Country") || ruleTree.label().value().contains("Result")
	...
	public QueryComponent setComponent(Tree tree) {
		...
		for (Tree temp : leaves) {
			if (temp.label().value().contains("---")) {
				...
				} else if (arrStr[0].equals("Competition")) {
	...
	public Tree createModelTree() {
		Tree modelTree = parser.getTree("Did Groothuis win gold in Speedskating?");
		changeLeaf(modelTree);
		modelRuleTree(modelTree);
		...
		int count = 0;
		// Parsing the input sentences and their corresponding manual tags and
		// inputting them into the PARSETREE
		for (String temp : sentences) {
			temp = temp.toLowerCase();
			// There is a input sentence but no available manual tagging -> Omit
			// parsing
			...
			addCorpusInput(objMapList, treeTemp); // Adding a sentence and its
													// corpus tag into the tree
			addSentenceToModel(treeTemp, modelTree);
			count++;
		}
		return modelTree;
	}

	public util.Result parseSentence(String question) {

		question = question.toLowerCase();
		Tree questionTree = parser.getTree(question);
		
		// parsing the question to identify its tags
		getDBMap(questionTree, modelTree);
		
		// Converting from output Tree to Query Component
		QueryComponent objComponent = setComponent(questionTree);
